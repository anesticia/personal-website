...
function createWebGLRenderer(canvas: HTMLCanvasElement, gl: WebGL2RenderingContext, variant: SimulationVariant): StateRenderer {
...
function createCanvasRenderer(canvas: HTMLCanvasElement, context: CanvasRenderingContext2D, variant: SimulationVariant): StateRenderer {
    ...
    render(state, width, height) {
      ...
      for (let cell = 0; cell < width * height; cell += 1) {
        ...
        if (variant === "publication") {
          ...
          data[offset + 1] = clamp(117 - outline * 91 - field * 6, 0, 255);
          data[offset + 2] = clamp(99 - outline * 82 - field * 5, 0, 255);
        }
        data[offset + 3] = 255;
      }
...
export function ReactionDiffusionCanvas({ variant = "hero", minimumFieldAspect = 0.35 }: { variant?: SimulationVariant; minimumFieldAspect?: number }) {
  ...
  useEffect(() => {
    ...
    const webgl = canvas.getContext("webgl2", { alpha: false, antialias: false, powerPreference: "high-performance", preserveDrawingBuffer: true });
    const fallbackContext = webgl ? null : canvas.getContext("2d", { alpha: false });
    if (!webgl && !fallbackContext) return;
    let renderer = webgl ? createWebGLRenderer(canvas, webgl, variant) : createCanvasRenderer(canvas, fallbackContext!, variant);
